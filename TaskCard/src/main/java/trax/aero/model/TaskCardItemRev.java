...
/**
 * The persistent class for the TASK_CARD_ITEM_REV database table.
 * 
 */
@Entity
@Table(name="TASK_CARD_ITEM_REV")
@NamedQuery(name="TaskCardItemRev.findAll", query="SELECT t FROM TaskCardItemRev t")
public class TaskCardItemRev implements Serializable {
	private static final long serialVersionUID = 1L;

	@EmbeddedId
	private TaskCardItemRevPK id;
	...
	@ManyToOne
	@JoinColumns({
		@JoinColumn(name="REVISION_CODE", referencedColumnName="REVISON" , insertable=false, updatable=false),
		@JoinColumn(name="TASK_CARD", referencedColumnName="TASK_CARD" , insertable=false, updatable=false)
		})
	private TaskCardRev taskCardRev;

	public TaskCardItemRev() {
	}

	public TaskCardItemRevPK getId() {
		return this.id;
	}

	public void setId(TaskCardItemRevPK id) {
		this.id = id;
	}
