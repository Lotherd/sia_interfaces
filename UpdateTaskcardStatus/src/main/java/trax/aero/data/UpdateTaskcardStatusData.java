...
public class UpdateTaskcardStatusData {
	...
	public ArrayList<I74_Request> getTaskCards() throws Exception
	{
		...
		+ "WTC.TASK_CARD TaskCard,WTC.WO,WTC.modified_by,WTC.modified_date FROM WO_TASK_CARD WTC JOIN WO W ON W.WO = WTC.WO WHERE WTC.INTERFACE_MODIFIED_DATE IS NOT NULL AND WTC.INTERFACE_TRANSFERRED_DATE IS NOT NULL AND WTC.INTERFACE_STEP IS NULL AND WTC.REFERENCE_TASK_CARD IS NOT NULL  AND  W.STATUS = 'OPEN' AND WTC.STATUS IN ('OPEN','CLOSED' , 'CANCEL') AND (WTC.non_routine = 'N' OR WTC.non_routine = 'Y' OR WTC.non_routine IS NULL)";
